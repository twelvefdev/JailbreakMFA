 ...
 * IN THE SOFTWARE.
 */
CRunMvtbox2daxial.B2FLAG_ROTATE = 0x0001;
CRunMvtbox2daxial.B2FLAG_FINECOLLISIONS = 0x0004;
function CRunMvtbox2daxial() {
    //call chain
    CRunMBase.call(this);
...
CRunMvtbox2daxial.prototype = {
        ...
        initialize: function (file) {
            ...
            this.m_length = file.readAInt();
            this.m_elasticity = file.readAInt() / 100.0;
            this.m_frequency = file.readAInt() / 100.0 * this.rh.rhApp.gaFrameRate / (2.0 * 10.0);
            this.m_offsetX = file.readAInt();
            this.m_offsetY = file.readAInt();
            this.m_torque = file.readAInt() / 100.0 * 35;
            this.m_damping = file.readAInt() / 100.0;
        ...
        CreateBody: function () {
            ...
            this.CreateFixture();
            var x = Math.floor(this.ho.hoX - this.m_length * Math.cos(this.m_angle * Box2D.Common.b2Settings.b2_pi / 180.0));
            var y = Math.floor(this.ho.hoY + this.m_length * Math.sin(this.m_angle * Box2D.Common.b2Settings.b2_pi / 180.0));
            this.m_bodyAxe = this.m_base.rCreateBody(Box2D.Dynamics.b2Body.b2_staticBody, x, y, 0, 0, null, 0, 0);
            this.m_base.rBodyCreateBoxFixture(this.m_bodyAxe, null, x, y, 16, 16, 0, 0, 0);
            var xOffset = Math.floor(this.m_offsetX * Math.cos(this.m_angle) - this.m_offsetY * Math.sin(this.m_angle));
            var yOffset = Math.floor(this.m_offsetX * Math.sin(this.m_angle) + this.m_offsetY * Math.cos(this.m_angle));
            this.m_base.rCreateDistanceJoint(this.m_bodyAxe, this.m_body, this.m_elasticity, this.m_frequency, xOffset, yOffset);        // TODO faire la rotation
            this.m_body.SetAngularVelocity(-this.m_torque);
            this.m_body.SetAngularDamping(Math.max(this.m_damping, 0.001));
        ...
        move: function () {
            ...
            if (this.m_flags & CRunMvtbox2daxial.B2FLAG_FINECOLLISIONS) {
                this.collisions();
            }
        ...
        SetCurrentAngle: function (angle) {
            if (angle != this.m_previousAngle) {
                ...
                this.ho.roc.rcChanged = true;
                if (this.m_flags & CRunMvtbox2daxial.B2FLAG_ROTATE) {
        ...
        SetPos: function (x, y) {
            var oBody = {};
            var oAxe = {};
            this.m_base.rGetBodyPosition(this.m_body, oBody);
            this.m_base.rGetBodyPosition(this.m_bodyAxe, oAxe);
            var deltaX = oAxe.x - oBody.x;
            var deltaY = oAxe.y - oBody.y;
        ...
        setDir: function (dir) {
            ...
            if (!this.m_started) {
                this.SetCurrentAngle(dir * 11.25);
            }
        },
...
CServices.extend(CRunMBase, CRunMvtbox2daxial);