 ...
 * IN THE SOFTWARE.
 */
CRunMvtbox2dspaceship.B2FLAG_ROTATE = 0x0001;
CRunMvtbox2dspaceship.B2FLAG_FINECOLLISIONS = 0x0004;
CRunMvtbox2dspaceship.ACCMULT = 0.2;
CRunMvtbox2dspaceship.DECMULT = 7.5;
CRunMvtbox2dspaceship.ANGLEMULT = 3.0;

function CRunMvtbox2dspaceship() {
    //call chain
    CRunMBase.call(this);
    ...
    this.m_initialImpulse = 0;
    this.m_deceleration = 0;
    this.m_shape = 0;
    this.m_flags = 0;
    this.m_angleSpeed = 0;
    this.m_autoRotateLeft = 0;
    this.m_autoRotateRight = 0;
    this.m_autoReactor = 0;
    this.m_previousX = 0;
    this.m_previousY = 0;
    this.m_player = 0;
    this.m_addVX = 0;
...
CRunMvtbox2dspaceship.prototype = {
        ...
        initialize: function (file) {
            ...
            var power = file.readAInt();
            this.m_power = power / (100.0 * CRunMvtbox2dspaceship.ACCMULT);
            this.m_thruster = file.readAShort();
            this.m_angleSpeed = 50.0 * file.readAInt() / (100.0 * CRunMvtbox2dspaceship.ANGLEMULT);
            this.m_initialImpulse = file.readAInt() / 100.0 * 10.0;
            this.m_deceleration = file.readAInt() / (100.0 * CRunMvtbox2dspaceship.DECMULT);
            this.m_player = file.readAShort();
        ...
        CreateBody: function () {
            ...
            this.m_body = this.m_base.rCreateBody(Box2D.Dynamics.b2Body.b2_dynamicBody, this.ho.hoX, this.ho.hoY, this.m_currentAngle, this.m_gravity, this, 0, 0);
            if (!this.ho.roa) {
                this.m_shape = 0;
            ...
            var mass = this.m_body.GetMass();
            this.m_power *= mass * 2.0;
            this.m_initialImpulse *= mass * 2.0;
            if (this.m_initialImpulse > 0) {
                this.m_base.rBodyApplyImpulse(this.m_body, this.m_initialImpulse, this.m_currentAngle + 180.0);
            }
        ...
        move: function () {
            ...
            this.m_currentAngle = this.m_base.rBodyGetAngle(this.m_body);
            if ((j & 15) != 0 || (this.m_autoRotateRight || this.m_autoRotateLeft)) {
                if (j & 0x04 || this.m_autoRotateLeft) {
                    this.m_autoRotateLeft = false;
                    this.m_currentAngle += this.m_angleSpeed;
                    this.m_base.rBodySetAngularVelocity(this.m_body, 0);
                }
                if (j & 0x08 || this.m_autoRotateRight) {
                    this.m_autoRotateRight = false;
                    this.m_currentAngle -= this.m_angleSpeed;
                    this.m_base.rBodySetAngularVelocity(this.m_body, 0);
                }
            }
            if (this.m_currentAngle > 360.0) {
                this.m_currentAngle -= 360.0;
            }
            ...
            if ((j & mask)) {
                this.m_base.rBodyAddLinearVelocity(this.m_body, this.m_power, this.m_currentAngle);
            ...
            if (this.m_flags & CRunMvtbox2dspaceship.B2FLAG_FINECOLLISIONS) {
                this.collisions();
            }
        ...
        SetCurrentAngle: function () {
            if (this.m_currentAngle != this.m_previousAngle) {
                ...
                this.ho.roc.rcChanged = true;
                if (this.m_flags & CRunMvtbox2dspaceship.B2FLAG_ROTATE) {
        ...
        // ------------------------
        setAcc: function (acc) {
            this.m_power = acc / (100.0 * CRunMvtbox2dspaceship.ACCMULT);
        },

        setDec: function (dec) {
            this.m_deceleration = dec / (100.0 * CRunMvtbox2dspaceship.DECMULT);
        },

        setRotSpeed: function (speed) {
            this.m_angleSpeed = 50.0 * speed / (100.0 * CRunMvtbox2dspaceship.ANGLEMULT);
        },
        ...
        getAcceleration: function () {
            return Math.floor(this.m_power * (100.0 * CRunMvtbox2dspaceship.ACCMULT));
        },

        getDeceleration: function () {
            return Math.floor(this.m_deceleration * (100.0 * CRunMvtbox2dspaceship.DECMULT));
        },
...
CServices.extend(CRunMBase, CRunMvtbox2dspaceship);