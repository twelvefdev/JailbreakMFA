 ...
 * IN THE SOFTWARE.
 */
CRunMvtbox2dracecar.B2FLAG_ROTATE = 0x0001;
CRunMvtbox2dracecar.B2FLAG_FINECOLLISIONS = 0x0008;
CRunMvtbox2dracecar.RCFLAG_REVERSE = 0x0002;
CRunMvtbox2dracecar.ACCMULT = 1.0;
CRunMvtbox2dracecar.DECMULT = 1.0;
CRunMvtbox2dracecar.ROTMULT = 15.0;
CRunMvtbox2dracecar.SPEEDMULT = 0.20;

function CRunMvtbox2dracecar() {
    //call chain
    CRunMBase.call(this);
...
CRunMvtbox2dracecar.prototype = {
        ...
        initialize: function (file) {
            file.skipBytes(1);
            this.m_angleCalculation = this.DirAtStart(file.readAInt()) * 180.0 / 16.0;
            ...
            this.m_speed = speed * CRunMvtbox2dracecar.SPEEDMULT;
            this.m_acceleration = file.readAInt() / (100.0 * CRunMvtbox2dracecar.ACCMULT);
            this.m_deceleration = file.readAInt() / (100.0 * CRunMvtbox2dracecar.DECMULT);
            this.m_angleSpeed = file.readAInt() / 100.0 * CRunMvtbox2dracecar.ROTMULT;
        ...
        CreateBody: function () {
            ...
            this.m_body = this.m_base.rCreateBody(Box2D.Dynamics.b2Body.b2_dynamicBody, this.ho.hoX, this.ho.hoY, this.m_angle, this.m_gravity, this, CRunMBase.CBFLAG_FIXEDROTATION, 0);
            if (!this.ho.roa) {
                this.m_shape = 0;
        ...
        move: function () {
            ...
            if ((j & 2) != 0) {
                var baseSpeed = 0;
                if ((this.m_flags & CRunMvtbox2dracecar.RCFLAG_REVERSE) != 0) {
            ...
            if (this.m_flags & CRunMvtbox2dracecar.B2FLAG_FINECOLLISIONS) {
                this.collisions();
            }
        ...
        SetCurrentAngle: function () {
            if (this.m_currentAngle != this.m_previousAngle) {
                ...
                this.ho.roc.rcChanged = true;
                if (this.m_flags & CRunMvtbox2dracecar.B2FLAG_ROTATE) {
        ...
        getAcceleration: function () {
            return Math.floor(this.m_acceleration * (100.0 * CRunMvtbox2dracecar.ACCMULT));
        },

        getDeceleration: function () {
            return Math.floor(this.m_deceleration * (100.0 * CRunMvtbox2dracecar.DECMULT));
        },

        setSpeed: function (speed) {
            var speedf = speed * CRunMvtbox2dracecar.SPEEDMULT;
            speedf = Math.min(speedf, this.m_speed);
            speedf = Math.max(speedf, 0);
            this.m_currentSpeed = speedf;
        },
...
CServices.extend(CRunMBase, CRunMvtbox2dracecar);