...
function CRunHTML5Video() {
    ...
    //call self
    this.resizeToVideo = false;
    this.video = null;
    this.videoCanvas = null;
    this.videoCanvasContext = null;
    this.videoSpriteUpdateRate = 0;
    this.videoImageContainer = null;
    this.videoLastUpdatedFrame = -1;
    this.videoKeepAspect = true;
    this.url = null;
    this.flags = 0;
    this.bLoaded = false;
    this.lastX = -1;
...
CRunHTML5Video.prototype = {
    ...
    createRunObject: function (file, cob, version) {
        ...
        var displayAsSprite = (this.flags & CRunHTML5Video.FLAG_DISPLAY) != 0;
        ...
        if (displayAsSprite) {
            ...
            this.videoCanvas.width = this.ho.hoImgWidth;
            this.videoCanvas.height = this.ho.hoImgHeight;
            this.videoCanvas.style.width = this.ho.hoImgWidth + 'px';
            this.videoCanvas.style.height = this.ho.hoImgHeight + 'px';
        ...
        if (this.url != "") {
            ...
            this.video.addEventListener("loadeddata", function (e) {
                that.resizeToVideo = (that.flags & CRunHTML5Video.FLAG_RESIZETOVIDEO) != 0;
                that.bLoaded = true;
        ...
        switch (Runtime.deviceType) {
            case CRuntime.DEVICE_PHONE:
                this.videoSpriteUpdateRate = Math.max(Runtime.frameRate/2,30);
                break;
            default:
                this.videoSpriteUpdateRate = 0;
                break;
        ...
        this.video.className = 'fusionRunControlHTML5Video';

        this.setElement(this.video, displayAsSprite);
    ...
    destroyRunObject: function () {
        ...
        //call self
        if (this.videoImageContainer != null) {
            this.videoImageContainer.free();
            this.videoImageContainer = null;
        }
    },

    displayRunObject: function (context, xDraw, yDraw) {
        //display video as sprite
        if ((this.flags & CRunHTML5Video.FLAG_DISPLAY) != 0 && !this.video.ended) {
            //do we need to update the texture

            var currentTime = this.video.currentTime;
            var currentFrame = Math.floor(currentTime * this.videoSpriteUpdateRate);

            //update canvas
            if (this.videoSpriteUpdateRate == 0 || currentFrame != this.videoLastUpdatedFrame) {
                this.videoLastUpdatedFrame = currentFrame;
                this.videoCanvasContext.drawImage(this.video, 0, 0, this.ho.hoImgWidth, this.ho.hoImgHeight);

                //update image container
                if (this.videoImageContainer == null) {
                    this.videoImageContainer = context.createImageContainer(this.videoCanvas, this.ho.hoImgWidth, this.ho.hoImgHeight);
                } else {
                    this.videoImageContainer.update(this.videoCanvas, this.ho.hoImgWidth, this.ho.hoImgHeight);
                }
            }

            //figure out render dimensions
            var renderX = 0;
            var renderY = 0;
            var renderWidth = this.ho.hoImgWidth;
            var renderHeight = this.ho.hoImgHeight;
            if (this.videoKeepAspect) {
                var scaleFactor = (this.video.videoWidth > this.video.videoHeight) ? this.ho.hoImgWidth / this.video.videoWidth : this.ho.hoImgHeight / this.video.videoHeight;

                renderWidth = this.video.videoWidth * scaleFactor;
                renderHeight = this.video.videoHeight * scaleFactor;

                renderX = (this.ho.hoImgWidth / 2) - (renderWidth / 2);
                renderY = (this.ho.hoImgHeight / 2) - (renderHeight / 2);
            }
            ...
            if (renderX != 0 || renderY != 0) {
                context.renderFilledRect(x, y, this.ho.hoImgWidth, this.ho.hoImgHeight, 0x000000);
            }

            //render video
            this.videoImageContainer.drawRect(context, x + renderX, y + renderY, renderWidth, renderHeight, 0, 0);
        }
    },

    handleRunObject: function () {
        if (this.resizeToVideo && this.video.videoWidth != 0 && this.video.videoHeight != 0) {
            this.resizeToVideo = false;
            ...
            if (this.videoCanvas != null) {
                ...
                this.videoCanvas.style.width = this.video.videoHeigh + 'px';
                this.videoCanvas.style.height = this.video.videoHeigh + 'px';
            }
        }
...
CServices.extend(CRunControl, CRunHTML5Video);