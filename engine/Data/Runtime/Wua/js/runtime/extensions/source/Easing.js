...
CRunEasing.prototype = {
    ...
    handleRunObject: function () {
        ...
        for (key in this.controlled) {
            ...
            if (object != null) {
                if (moved.timeMode == 0) {
                    var diff = (Date.now() - moved.startTime);
    ...
    con_SpecificObjectStopped: function (cnd) {
        ...
        if (this.select == null) {
            this.select = new CObjectSelection(this.rh.rhApp);
        }
    ...
    filterMoving: function (object) {
        return this.controlled[this.GetFixedFromObject(object)] != null;
    },
    ...
    con_IsObjectMoving: function (cnd, isNegated) {
        ...
        return this.select.filterObjects(oiList, isNegated, this.filterMoving);
    },
    ...
    act_ReverseObject: function (object) {
        ...
        if (reversed.timeMode == 0) {
            var ms = Date.now();
            reversed.timespan = Math.floor(ms - reversed.startTime);
            reversed.startTime = ms;
        }
        else {
...
CServices.extend(CRunExtension, CRunEasing);
