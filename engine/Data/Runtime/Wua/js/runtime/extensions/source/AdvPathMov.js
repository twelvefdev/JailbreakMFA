...
CRunAdvPathMov.prototype = {
    ...
    action: function (num, act) {
        switch (num) {
            ...
            case CRunAdvPathMov.AID_LoadPath:
                //this.LoadPath(act.getParamExpString(this.rh, 0));
                break;
    ...
    LoadPath: function (filename) {
        //create file we can read from
        var file = this.rh.rhApp.openFile(filename, true);
        ...
        while(!file.isEOF()) {
            ...
            if(Currentpos > 1 && (Currentpos % 2 ) == 1) {
                Loadfloat = file.readAFloat();
            }

            if(special > 0) {
                if(Currentpos == 0){
                    this.mypointer.myvector.add(new CRunAdvPathMovPoints(Loadnumber, 0));
                }

                if(Currentpos == 1){
                    this.mypointer.theIterator = this.mypointer.myvector.get(vectorpos); 
                    this.mypointer.theIterator.Y = Loadnumber;
                }

                if(Currentpos > 1 && (Currentpos % 2) == 0) {
                    this.mypointer.theIterator = this.mypointer.myvector.get(vectorpos); 
                    this.mypointer.theIterator.Connections.add(new CRunAdvPathMovConnect(Loadnumber, 5));
                }

                if(Currentpos > 1 && (Currentpos % 2) == 1) {
                    this.mypointer.theIterator = this.mypointer.myvector.get(vectorpos); 
                    this.mypointer.theIterator.ConnectIterator = this.mypointer.theIterator.Connections.get(Math.round((Currentpos - 3) / 2));
                    this.mypointer.theIterator.ConnectIterator.Distance = Loadfloat + 20.0; 
                }
    ...
    expression: function (num) {
        switch (num) {
            case CRunAdvPathMov.EID_Findnode:
                return this.Findnode(this.ho.getExpParam(), this.ho.getExpParam(), this.ho.getExpParam());
    ...
    GetNodesSpeed: function (p1, p2) {
        ...
        //param1 contains the number inputed by the user
        //param2 contains the number inputed by the user
        if ((p1 >= 0) && (p1 < this.mypointer.myvector.size()) && (p2 >= 0) && (p2 < this.mypointer.myvector.size())) {
...
CServices.extend(CRunExtension, CRunAdvPathMov);
