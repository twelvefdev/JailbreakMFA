...
function CRunkcclock() {
    ...
    this.textBuffers = false;//does testBuffer contain an array of buffers?
};

CRunkcclock.prototype = {
    ...
    createRunObject: function (file, cob, version) {
        ...
        this.fontString = this.sFont.getCSS();
    ...
    destroyRunObject: function (bFast) {
        ...
        if (this.textContainer) {
            if (this.textBuffers) {
                //multiple
                for (var index = 0; index < this.textContainer.length; index++) {
                    this.textContainer[index].free();
                    this.textContainer[index] = null;
                }
            } else {
                //single
    ...
    RunDisplayAnalogTime: function (context, sHour, sMinutes, sSeconds, rc) {
        ...
        if (this.sAnalogClockMarkerType != 2) {
            ...
            if (this.textContainer == null) {
                this.textContainer = new Array(13);
                this.textBuffers = true;
            }

            var updateTextBuffer = false;
            for (a = 1; a < 13; a++) {
                var x, y;
                ...
                //create text buffer
                if (!this.textContainer[a]) {
                    updateTextBuffer = true;
                    this.textContainer[a] = app.createTextContainer();
                } else {
                    updateTextBuffer = false;
                }

                //get text dimensions
                textWidth = this.textContainer[a].getTextWidth(szString, this.sFont);
                ...
                //update text buffer with calculated details
                if (updateTextBuffer) {
                    this.textContainer[a].set(szString, this.sFont, this.crFont, CRendererTextContainer.LEFT | CRendererTextContainer.TOP, textWidth, textHeight);
                }
                ...
                this.textContainer[a].draw(context, X, Y);
            }
        }

        // Draw border if needed
        if (true == this.sClockBorder) {
            context.renderOutlineEllipse(pntPoints[0].x - sRayon, pntPoints[0].y - sRayon, sRayon * 2, sRayon * 2, 1, this.crFont, 0, 0);
            context.renderOutlineEllipse(pntPoints[0].x - sRayon, pntPoints[0].y - sRayon, sRayon * 2 + 1, sRayon * 2 + 1, 1, this.crFont, 0, 0);
        }
    },
    ...
    RunDisplayDigitalTime: function (context, szTime, rc) {
        ...
        if (this.textContainer == null) {
            ...
            this.textBuffers = false;
        }

        //get dimensions
        var width = this.textContainer.getTextWidth(szTime, this.sFont);
        var height = this.sFont.lfHeight;
        var X = rc.left + (rc.right - rc.left) / 2 - width / 2;
        var Y = rc.top + (rc.bottom - rc.top) / 2 - height / 2;

        //update text buffer
        this.textContainer.set(szTime, this.sFont, this.crFont, CRendererTextContainer.LEFT | CRendererTextContainer.TOP, width, height);

        //render the text
        this.textContainer.draw(context, Math.floor(X), Math.floor(Y), 0, 0);

        //border
        if (true == this.sClockBorder) {
            context.renderOutlineRect(rc.left + 1, rc.top + 1, rc.right - rc.left, rc.bottom - rc.top, this.crFont, 2, 0, 0);
        }
    },
    ...
    RunDisplayCalendar: function (context, szDate, rc) {
        ...
        //get dimensions
        var width = this.textContainer.getTextWidth(szDate, this.sFont);
        var height = this.sFont.lfHeight;
        var X = rc.left + (rc.right - rc.left) / 2 - width / 2;
        var Y = rc.top + (rc.bottom - rc.top) / 2 - height / 2;

        //update text buffer
        this.textContainer.set(szDate, this.sFont, this.crFont, CRendererTextContainer.LEFT | CRendererTextContainer.TOP, width, height);
...
CServices.extend(CRunExtension, CRunkcclock);
