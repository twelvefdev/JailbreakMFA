...
CRunLayer.prototype = {
            ...
            if (roPtr1.ros != null && param1 == FValue1) {
                ...
                for (roPtr2 = this.ho.getFirstObject(); roPtr2 != null; roPtr2 = this.ho.getNextObject()) {
                    ...
                    if (roPtr2.ros != null && param2 == FValue2) {
                        ...
                        var i1 = roPtr1.getZOrder();
                        var i2 = roPtr2.getZOrder();
                        if (i1 >= 0 && i2 >= 0) {
                            if (i1 > i2) {
                                return true;
                            }
    ...
    cndBetween: function (cnd) {
        ...
        if (sprPtr1 != null) {
            ...
            if ((sprPtr1 != null) && (sprPtr2 != null) && (sprPtr3 != null)) {
                // MMF2
                var n1 = sprPtr1.getZOrder();
                var n2 = sprPtr2.getZOrder();
                var n3 = sprPtr3.getZOrder();
                if (n1 >= 0 && n2 >= 0 && n3 >= 0) {
    ...
    actBackOneRout: function (param1) {
        ...
        if ((sprPtr1 = this.lyrGetSprite(param1)) != null) {
            var index = sprPtr1.getZOrder();
            sprPtr1.setZOrder(index - 1);
        }
    },
    ...
    actSetLayerX: function (act) {
        ...
        if (param1 > 0 && param1 <= this.ho.hoAdRunHeader.rhFrame.nLayers) {
            ...
            if (pLayer.x != newX || pLayer.dx != 0) {
                ...
                this.ho.hoAdRunHeader.rh3Scrolling |= CRun.RH3SCROLLING_SCROLL;
            }
        }
    },
    ...
    lyrSwapSpr: function (sp1, sp2) {
        ...
        var i1 = sp1.getZOrder();
        var i2 = sp2.getZOrder();
        if (i1 >= 0 && i2 >= 0) {
            sp1.setZOrder(i2);
            sp2.setZOrder(i1);
        }
    ...
    lyrSortBy: function (flag, altDefaultVal, altValue) {
        //this will take the valid objects on a layer and then figure out teh zOrder value for each one.
        //it will then modify the objects zOrder value, and let the layer scene node rearrange its children by their zOrder.

        //check valid compare func
        var compareFunc = this._compareFlagToFunction(flag);
        if (compareFunc == null) {
            return;
        }

        //lock the layer first so that ordering over children will happen when we unlock
        var layerIndex = this.wCurrentLayer;
        var layer = this.ho.hoAdRunHeader.rhFrame.layers[layerIndex];

        layer.lockOrder();

        //create list of sorting items
        var sortList = new CArrayList();
        var sortItem;
        var sprite;

        for (sprite = this.ho.getFirstObject() ; sprite != null; sprite = this.ho.getNextObject()) {
            //sprite must be on the same layer
            if (sprite.ros != null && sprite.hoLayer == layerIndex) {
                sortItem = new CRunLayerSortData();

                if (sprite.getZOrder() >= 0) {
                    //build data item
                    sortItem.object = sprite;
                    sortItem.sprX = sprite.hoX;
                    sortItem.sprY = sprite.hoY;
                    sortItem.sprAlt = altDefaultVal;

                    if (sprite.rov != null && sprite.rov.rvValues != null && sprite.rov.rvValues[altValue] != null) {
                        sortItem.sprAlt = sprite.rov.rvValues[altValue];
                    }
        ...
        //do the sort routine using the compare func specified by the flag paramter
        var count = 0;
        var n;
        var item1;
        var item2;
        ...
        do {
            count = 0;
            for (n = 0; n < sortList.size() - 1; n++) {
                item1 = sortList.get(n);
                item2 = sortList.get(n + 1);

                if (compareFunc(item1, item2)) {
                    //swap these
                    sortList.set(n + 1, item1);
                    sortList.set(n, item2);
                    count++;
                }
            }
        } while (count != 0);

        //we now have a list of sprites where the top most one should be top most when renderered.

        //set the z order of each sprite in reverse order based on index in the sort list
        for (count = sortList.size() -1; count >= 0; count--) {
            sortItem = sortList.get(count);
            sprite = sortItem.object;
            sprite.setZOrder(count);
        }

        //now that we are finished we should unlock and let the scene nodes order
        layer.unlockOrder();

        return false;
    },

    _compareFlagToFunction: function(flag) {
        //return teh correct comparison function
        switch (flag) {
            case CRunLayer.X_UP:
                return this._compareOnSameLayerXUp;
            case CRunLayer.X_DOWN:
                return this._compareOnSameLayerXDown;
            case CRunLayer.Y_UP:
                return this._compareOnSameLayerYUp;
            case CRunLayer.Y_DOWN:
                return this._compareOnSameLayerYDown;
            case CRunLayer.ALT_UP:
                return this._compareOnSameLayerAltUp;
            case CRunLayer.ALT_DOWN:
                return this._compareOnSameLayerAltDown;
        }
    ...
    _compareOnSameLayerXUp: function(item1,item2) {
        return item1.sprX < item2.sprX;
    },

    _compareOnSameLayerXDown: function (item1, item2) {
        return item1.sprX > item2.sprX;
    },

    _compareOnSameLayerYUp: function (item1, item2) {
        return item1.sprY < item2.sprY;
    },

    _compareOnSameLayerYDown: function (item1, item2) {
        return item1.sprY > item2.sprY;
    },

    _compareOnSameLayerAltUp: function (item1, item2) {
        return item1.sprAlt < item2.sprAlt;
    },

    _compareOnSameLayerAltDown: function (item1, item2) {
        return item1.sprAlt > item2.sprAlt;
    },
        ...
        if (oilPtr1.oilEventCount == this.ho.hoAdRunHeader.rhEvtProg.rh2EventCount) {
            if (param2 != null) {
                ...
                for (i = 1; i <= oilPtr1.oilNumOfSelected; i++) {
                    ...
                    for (j = 1; j <= Loop2; j++) {
                        ...
                        if (DoLevel2) {
                            if ((roPtr2.hoNextSelected & 0x80000000) == 0) {
                                roPtr2 = this.ho.hoAdRunHeader.rhObjectList[roPtr2.hoNextSelected];
                                FValue2 = (roPtr2.hoCreationId << 16) + (roPtr2.hoNumber & 0xFFFF);
                            }
                        }
                        else {
                            if ((roPtr2.hoNumNext & 0x80000000) == 0) {
                    ...
                    if ((roPtr1.hoNextSelected & 0x80000000) == 0) {
            ...
            if (param2 != null) {
                ...
                for (i = 1; i <= oilPtr1.oilNObjects; i++) {
                    ...
                    if ((roPtr1.hoNumNext & 0x80000000) == 0) {
...
CServices.extend(CRunExtension, CRunLayer);
