...
CRunKcBoxB.prototype = {
    ...
    createRunObject: function (file, cob, version) {
        ...
        if (this.ho.hoAdRunHeader.rhApp.unicode) {
            textSize /= 2;
        }
        if (textSize != 0) {
        ...
        if ((this.rData_dwFlags & CRunKcBoxB.FLAG_CONTAINED) != 0) {
            ...
            if (this.rContNum != -1) {
                var rdPtrCont = pData.pContainers.get(this.rContNum);
        ...
        this.bShown = true;
        this.pLayer.planeBack.addChild(this.ho);

        return false;
    },

    destroyRunObject: function (bFast) {
        this.ho.removeFromParent();

        var rhPtr = this.ho.hoAdRunHeader;

        var pData = rhPtr.getStorage(this.ho.hoIdentifier);
    ...
    DisplayObject: function (context, idAppApp, rc, pText, hFnt) {
        ...
        if (this.rData_fillColor != CRunKcBoxB.COLOR_NONE) {
            ...
            if ((clr & CRunKcBoxB.COLORFLAG_RGB) != 0) {
                ...
                color = this.fromC(color);
            }
            else {
                color = this.myGetSysColor(clr);
            }
            context.renderFilledRect(x, y, w, h, color);
        }
        ...
        if ((this.rData_wImage != null)) {
            if (((this.rData_dwFlags & CRunKcBoxB.FLAG_HIDEIMAGE) == 0)) {
                ...
                if (bDisplayImage == true) {
                    ...
                    if (wc > 0 && hc > 0) {
                        if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_IMAGE_PATTERN) != 0) {
                            context.renderPatternRect(this.rData_wImage, xc, yc, wc, hc, 0, 0);
                        }
                        else {
                            if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_IMAGE_TOPLEFT) != 0) {
                                xc = x;
                                yc = y;
                            }
                            else if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_IMAGE_CENTER) != 0) {
                                xc = x + Math.floor((w - this.rData_wImage.width) / 2);
                                yc = y + Math.floor((h - this.rData_wImage.height) / 2);
                            }
                            context.renderImageWithSubPixelCorrection(this.rData_wImage, xc, yc, 0, 1, 1, 0, 0, null);
                        }
                    }
        ...
        // Text
        if ((pText.length != 0) && (this.rData_textColor != CRunKcBoxB.COLOR_NONE)) {
            ...
            if ((this.rData_dwFlags & CRunKcBoxB.FLAG_BUTTON) != 0 && (this.rData_dwFlags & (CRunKcBoxB.FLAG_BUTTON_PRESSED | CRunKcBoxB.FLAG_CHECKED)) != 0 && (this.rData_dwFlags & (CRunKcBoxB.FLAG_BUTTON | CRunKcBoxB.FLAG_CHECKBOX | CRunKcBoxB.FLAG_IMAGECHECKBOX)) != (CRunKcBoxB.FLAG_BUTTON | CRunKcBoxB.FLAG_CHECKBOX | CRunKcBoxB.FLAG_IMAGECHECKBOX)) {
                textLocation.left += 2;
                textLocation.top += 2;
            }

            if ((this.rData_dwFlags & CRunKcBoxB.FLAG_DISABLED) != 0) {
                clr = this.myGetSysColor(16);
                textLocation.left++;
                textLocation.top++;
                textLocation.right++;
                textLocation.bottom++;
                ...
                if ((this.rData_dwFlags & CRunKcBoxB.FLAG_HYPERLINK) != 0) {
                    if ((this.rData_dwFlags & (CRunKcBoxB.FLAG_BUTTON_HIGHLIGHTED | CRunKcBoxB.FLAG_BUTTON_PRESSED)) != 0) {
        ...
        if (bDisplayBorder == true) {
            if (color1 != CRunKcBoxB.COLOR_NONE) {
                if ((color1 & CRunKcBoxB.COLORFLAG_RGB) != 0) {
                    color1 &= ~CRunKcBoxB.COLOR_FLAGS;
                    color1 = this.fromC(color1);
                }
                else {
                    color1 = this.myGetSysColor(color1);
                }
                context.renderFilledRect(x, y, w - 1, 1, color1);
                context.renderFilledRect(x, y, 1, h - 1, color1);
            }
            if (color2 != CRunKcBoxB.COLOR_NONE) {
                if ((color2 & CRunKcBoxB.COLORFLAG_RGB) != 0) {
                    color2 &= ~CRunKcBoxB.COLOR_FLAGS;
                    color2 = this.fromC(color2);
                }
                else {
                    color2 = this.myGetSysColor(color2);
                }
                context.renderFilledRect(x, y + h - 1, w - 1, 1, color2);
                context.renderFilledRect(x + w - 1, y, 1, h, color2);
            }
        }
    },
    ...
    drawText: function (context, text, textLocation, font, flags, hyperlink, clr) {
        if (this.displayText) {
            ...
            var flags = CRendererTextContainer.WORDWRAP;

            if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_HCENTER) != 0) {
                flags |= CRendererTextContainer.CENTER;
            } else if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_RIGHT) != 0) {
                flags |= CRendererTextContainer.RIGHT;
            } else {
                flags |= CRendererTextContainer.LEFT;
            }

            if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_MULTILINE) != 0) {
                flags |= CRendererTextContainer.LINEBREAKS;
            }

            if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_NOPREFIX) == 0) {
                var n;
                var temp;
            ...
            this.textContainer.set(text, font, clr, flags, textLocation.right - textLocation.left, textLocation.bottom - textLocation.top);
            this.syText = this.textContainer.measureHeight();
        }
        var y = textLocation.top;
        if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_VCENTER) != 0) {
            y = Math.floor(textLocation.top + (textLocation.bottom - textLocation.top) / 2 - this.syText / 2);
        }
        if ((this.rData_dwFlags & CRunKcBoxB.ALIGN_BOTTOM) != 0) {
            y = textLocation.bottom - this.syText;
        }
        this.textContainer.draw(context, textLocation.left, y, 0, 0)
    },
...
CServices.extend(CRunExtension, CRunKcBoxB);
