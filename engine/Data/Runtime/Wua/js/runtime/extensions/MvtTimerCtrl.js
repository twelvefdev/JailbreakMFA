...
// of your extension.
CRunMvtTimerCtrl.CND_COMPAREBASE = 0;
CRunMvtTimerCtrl.CND_COMPAREMVTTIMERSTATE = 1;
...
CRunMvtTimerCtrl.CND_LAST = 2;

CRunMvtTimerCtrl.ACT_SETBASE = 0;
CRunMvtTimerCtrl.ACT_SETMVTTIMERSTATE = 1;
CRunMvtTimerCtrl.ACT_SETMVTTIMERPRECISION = 2;

CRunMvtTimerCtrl.EXP_GETBASE = 0;
CRunMvtTimerCtrl.EXP_GETMVTTIMERSTATE = 1;
CRunMvtTimerCtrl.EXP_BASETOPERCENT = 2;
CRunMvtTimerCtrl.EXP_PERCENTTOBASE = 3;
CRunMvtTimerCtrl.EXP_DELTATIME = 4;
CRunMvtTimerCtrl.EXP_BASETOFLOAT = 5;
CRunMvtTimerCtrl.EXP_FLOATTOBASE = 6;
...
// Global Variables
globalThis.PropsInitiated;
globalThis.DoubleMvtTimerCoef = false;
...
CRunMvtTimerCtrl.prototype = CServices.extend(new CRunExtension(),
{
    ...
    {
        return CRunMvtTimerCtrl.CND_LAST;
    ...
    createRunObject:function(file, cob, version)
    {
		...
		file.skipBytes(20)
		if (typeof globalThis.PropsInitiated === 'undefined' && file.readAByte() == 2)
		{
			globalThis.DoubleMvtTimerCoef = true;
		}
		globalThis.PropsInitiated == true;
    ...
    {
		if (globalThis.DoubleMvtTimerCoef)
		{
			this.rhPtr = this.ho.hoAdRunHeader;
			
			this.Delta = (this.rhPtr.rhTimer - this.TimerOld) / setPrecision(1000.0 / this.rhPtr.rhFrame.m_dwMvtTimerBase, 3);
			this.TimerOld = this.rhPtr.rhTimer;
			
			this.rh.rh4MvtTimerCoef = this.Delta;
		}
		
        return true;       // The object will be called next loop
    },
    ...
    {
		//This breaks it on frame restarts, and it's not really necessary.
    ...
    condition:function(num, cnd)
    {
        ...
        switch (num)
        {
            case CRunMvtTimerCtrl.CND_COMPAREBASE:
                var parameter = cnd.getParamExpression(this.rh, 0);
                return (parameter == this.ho.hoAdRunHeader.rhFrame.m_dwMvtTimerBase);
				break;
			case CRunMvtTimerCtrl.CND_COMPAREMVTTIMERSTATE:
				return (this.ho.hoAdRunHeader.rhFrame.leFlags & CRunFrame.LEF_TIMEDMVTS) != 0;
        }
        return false;
    },
    ...
    {   
        switch (num)
        {
            case CRunMvtTimerCtrl.ACT_SETBASE:
				this.ho.hoAdRunHeader.rhFrame.m_dwMvtTimerBase = act.getParamExpression(this.rh, 0);
                break;
			case CRunMvtTimerCtrl.ACT_SETMVTTIMERSTATE:
				if (act.getParamExpression(this.rh, 0) <= 0)
					this.ho.hoAdRunHeader.rhFrame.leFlags &= ~CRunFrame.LEF_TIMEDMVTS;
				else
					this.ho.hoAdRunHeader.rhFrame.leFlags |= CRunFrame.LEF_TIMEDMVTS;
				break;
            case CRunMvtTimerCtrl.ACT_SETMVTTIMERPRECISION:
				if (act.getParamExpression(this.rh, 0))
				{
					this.TimerOld = this.ho.hoAdRunHeader.rhTimer;
					globalThis.DoubleMvtTimerCoef = true;
				}
				else
					globalThis.DoubleMvtTimerCoef = false;
    ...
    expression: function(num)
    {
        switch (num)
        {
            case CRunMvtTimerCtrl.EXP_GETBASE: return this.ho.hoAdRunHeader.rhFrame.m_dwMvtTimerBase;
			case CRunMvtTimerCtrl.EXP_GETMVTTIMERSTATE:
				return ((this.ho.hoAdRunHeader.rhFrame.leFlags & CRunFrame.LEF_TIMEDMVTS) != 0 ? 1 : 0);
			case CRunMvtTimerCtrl.EXP_PERCENTTOBASE: return ((this.ho.getExpParam() / 100.0) * 60.0);
			case CRunMvtTimerCtrl.EXP_BASETOPERCENT: return ((this.ho.getExpParam() / 60.0) * 100.0);
			case CRunMvtTimerCtrl.EXP_DELTATIME:
				return ((this.ho.hoAdRunHeader.rhFrame.leFlags & CRunFrame.LEF_TIMEDMVTS) != 0 ? this.rh.rh4MvtTimerCoef : 1);
			case CRunMvtTimerCtrl.EXP_FLOATTOBASE: return this.ho.getExpParam() * 60.0;
			case CRunMvtTimerCtrl.EXP_BASETOFLOAT: return this.ho.getExpParam() / 60.0;
        }
        return 0;
