...
CTransFade.prototype = CServices.extend(new CTrans(), {
    ...
    stepDraw: function (flag) {
        //is this the first time we are drawing the transition?
        if (this.m_starting) {
            this.m_starting = false;
        }
        ...
        //TODO: fade in / out is identical.. need to fix this...

        //what fade are we doing?
        if ((flag & CTrans.TRFLAG_FADEIN) != 0) {
            //fade in
            this.destination.start();
            this.destination.setAlpha(1.0);
            this.blit(this.source1);
            fadeCoef = this.getDeltaTime() / this.m_duration;
            this.destination.setAlpha(fadeCoef);
            this.blit(this.source2);
