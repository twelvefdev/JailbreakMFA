...
CRunJoystickControl.prototype = CServices.extend(new CRunExtension(),
	{
		...
		destroyRunObject: function (bFast)
		{
			...
			{
			    //            this.ho.hoAdRunHeader.stopJoystickAcc();
			}
		},
		...
		startStopTouch:     function (act)
		{
			...
			if ((rhApp.frame.html5Options & CRunFrame.HTML5FOPT_JOYSTICK_DPAD) != 0)
			{
			...
			if ((flags & (CJoystick.JFLAG_FIRE1 | CJoystick.JFLAG_FIRE2 | CJoystick.JFLAG_JOYSTICK)) != 0)
			{
				...
				{
				    rhApp.joystick.setXPosition(CJoystick.JFLAG_JOYSTICK, this.xJoystick);
				}
				else
				{
				    this.xJoystick = rhApp.joystick.imagesX[CJoystick.KEY_JOYSTICK];
				}

				if (this.yJoystick != CRunJoystickControl.POS_NOTDEFINED)
				{
				    rhApp.joystick.setYPosition(CJoystick.JFLAG_JOYSTICK, this.yJoystick);
				}
				else
				{
				    this.yJoystick = rhApp.joystick.imagesY[CJoystick.KEY_JOYSTICK];
				}

				if (this.xFire1 != CRunJoystickControl.POS_NOTDEFINED)
				{
				    rhApp.joystick.setXPosition(CJoystick.JFLAG_FIRE1, this.xFire1);
				}
				else
				{
				    this.xFire1 = rhApp.joystick.imagesX[CJoystick.KEY_FIRE1];
				}

				if (this.yFire1 != CRunJoystickControl.POS_NOTDEFINED)
				{
				    rhApp.joystick.setYPosition(CJoystick.JFLAG_FIRE1, this.yFire1);
				}
				else
				{
				    this.yFire1 = rhApp.joystick.imagesY[CJoystick.KEY_FIRE1];
				}

				if (this.xFire2 != CRunJoystickControl.POS_NOTDEFINED)
				{
				    rhApp.joystick.setXPosition(CJoystick.JFLAG_FIRE2, this.xFire2);
				}
				else
				{
				    this.xFire2 = rhApp.joystick.imagesX[CJoystick.KEY_FIRE2];
				}

				if (this.yFire2 != CRunJoystickControl.POS_NOTDEFINED)
				{
				    rhApp.joystick.setXPosition(CJoystick.JFLAG_FIRE2, this.yFire2);
				}
				else
				{
				    this.yFire2 = rhApp.joystick.imagesY[CJoystick.KEY_FIRE2];
				}
				this.bJoystick = true;
			}
			else
		...
		setJoyPosition:   function (act)
		{
			...
			if (this.bJoystick && position.found)
			{
		...
		{
		    this.ho.hoAdRunHeader.rhJoystickMask = act.getParamExpression(this.rh, 0);
		},
