...
CRunkchisc.prototype = CServices.extend(new CRunExtension(),
	{
		...
		createRunObject:  function (file, cob, version)
		{
			...
			this.iniName = CServices.parseName(this.iniName);
		...
		createScores: function (context) {
		    ...
		    if (this.scoreSurfaces != null) {
		        for (a = 0; a < this.NbScores; a++) {
		            if (this.scoreSurfaces[a] == null)
		                this.scoreSurfaces[a] = new CTextSurface(this.rh.rhApp, this.ho.hoImgWidth / 4, this.ho.hoImgHeight / this.NbScores);
		            if (this.scoreSurfaces[a] != null)
		                this.scoreSurfaces[a].setText(this.Scores[a].toString(), ((this.Flags & CRunkchisc.SCR_NAMEFIRST) == 0) ? CServices.DT_LEFT | CServices.DT_TOP : CServices.DT_RIGHT | CServices.DT_TOP, null, this.Logfont, this.Colorref);
		        }
		    }
		},
		displayRunObject: function (context, xDraw, yDraw)
		{
			...
			//var x = this.xSave + this.ho.pLayer.x + xDraw;
			//var y = this.ySave + this.ho.pLayer.y + yDraw;
			var a;
			...
			if (this.textSurfaces != null)
			{
				for (a = 0; a < this.NbScores; a++)
				{
				    this.textSurfaces[a] = new CTextSurface(this.rh.rhApp, ((this.Flags & CRunkchisc.SCR_DONTDISPLAYSCORES) != 0) ? this.ho.hoImgWidth : (this.ho.hoImgWidth * 3)/4, this.ho.hoImgHeight / this.NbScores);
				    this.textSurfaces[a].setText(localNames[a],
                        ((this.Flags & (CRunkchisc.SCR_NAMEFIRST | CRunkchisc.SCR_DONTDISPLAYSCORES)) == 0) ? CServices.DT_RIGHT | CServices.DT_TOP : CServices.DT_LEFT | CServices.DT_TOP,
                        null, this.Logfont, this.Colorref);
				}
			}
			...
			else
			{
				...
				if (0 != (this.Flags & CRunkchisc.SCR_NAMEFIRST))
				{
					// Compute coordinates
					rc.left = x;
					//rc.right = rc.left + ((this.ho.hoImgWidth * 3) / 4);
					rc.top = y;
					...
					// Compute coordinates
					rc.left = x + ((this.ho.hoImgWidth * 3) / 4);
					rc.top = y;
				...
				else
				{
					...
					// Compute coordinates
					rc.left = x + (this.ho.hoImgWidth / 4);
					rc.right = x + this.ho.hoImgWidth;
					rc.top = y;
					//rc.bottom = rc.top + (this.ho.hoImgHeight / this.NbScores);
					...
					for (a = 0; a < this.NbScores; a++)
					{
					    //var width = this.textSurfaces[a].measureText(localNames[a], this.Logfont).width;
