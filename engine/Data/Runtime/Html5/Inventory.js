...
CRunInventory.prototype = CServices.extend(new CRunExtension(),
	{
		...
		{
			var spriteX = this.ho.hoX - this.rh.rhWindowX + this.ho.pLayer.x;
			var spriteY = this.ho.hoY - this.rh.rhWindowY + this.ho.pLayer.y;
						...
						if (o >= this.position && o < this.position + this.nLines * this.nColumns)
						{
							...
							if (o == this.position + this.xCursor + this.yCursor * this.nColumns)
							{
								if ((this.flags & CRunInventory.IFLAG_FORCECURSOR) != 0 && this.cursorType > 0)
								{
									...
									{
										context.renderRect(rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, this.cursorColor, 1, 0, 0);
									}
									else
									{
										context.renderSolidColor(rc.left, rc.top, rc.right - rc.left, rc.bottom - rc.top, this.cursorColor, 0, 0);
									}
								}
							}
							...
							if (this.bUpdateList)
							{
								if (this.maximum > 1)
								{
									...
									if (this.textSurfaces[count] == null)
									{
										this.textSurfaces[count] = new CTextSurface(this.rh.rhApp, this.itemSx, this.itemSy);
									}
									this.textSurfaces[count].resize(this.itemSx, this.itemSy);
									var sxText = this.textSurfaces[count].measureText(text, this.logFont);
									var syText = this.logFont.getHeight();

									if ((this.textAlignment & 0x00000001) != 0)       // TEXT_ALIGN_LEFT)
									{
										dtFlags = CServices.DT_LEFT;
										this.textsSurfaces[count].x = column * this.itemSx;
									...
									else if ((this.textAlignment & 0x00000002) != 0)  //TEXT_ALIGN_HCENTER)
									{
										...
										this.textSurfaces[count].x = column * this.itemSx;
									}
									else											// (this.textAlignment&TEXT_ALIGN_RIGHT)
									{
										xObject = column * this.itemSx;
										dtFlags = CServices.DT_LEFT;
										this.textSurfaces[count].x = xObject + hoPtr.hoImgWidth + CRunInventory.SX_SEPARATION;
									}
									if ((this.textAlignment & 0x00000008) != 0)       //TEXT_ALIGN_TOP)
									{
										sy = this.ho.hoImgHeight + CRunInventory.SY_SEPARATION + syText;
										this.textSurfaces[count].y = line * this.itemSy + Math.floor(this.itemSy / 2) - Math.floor(sy / 2);
										yObject = this.textSurfaces[count].y + syText + CRunInventory.SY_SEPARATION;
									}
									else if ((this.textAlignment & 0x00000010) != 0)  //TEXT_ALIGN_VCENTER)
									{
										this.textSurfaces[count].y = line * this.itemSy + Math.floor(this.itemSy / 2) - Math.floor(syText / 2) - Math.floor(syText / 6);
									...
									else											// (this.textAlignment&TEXT_ALIGN_BOTTOM)
									{
										...
										this.textSurfaces[count].y = yObject + hoPtr.hoImgHeight + CRunInventory.SY_SEPARATION;
									}

									if (pItem.GetQuantity() >= this.displayQuantity)
										this.textSurfaces[count].visible = true;
									else
										this.textSurfaces[count].visible = false;

									this.textSurfaces[count].manualClear(0);
									this.textSurfaces[count].manualDrawText(text, dtFlags, null, this.fontColor, this.logFont, 0, 0);

									this.SetPosition(hoPtr, this.ho.hoX + xObject, this.ho.hoY + yObject);
								}
							}
							if (this.textSurfaces[count] != null && this.textSurfaces[count].visible)
							{
								this.textSurfaces[count].draw(context, this.textSurfaces[count].x + spriteX, this.textSurfaces[count].y + spriteY);
							}
						}
						else
						{
				...
				if ((this.flags & CRunInventory.IFLAG_DISPLAYGRID) != 0)
				{
					...
					for (y = 0; y < this.nLines; y++)
					{
						for (x = 0; x < this.nColumns; x++)
						{
							...
							context.renderRect(rc.left + spriteX, rc.top + spriteY, rc.right - rc.left, rc.bottom - rc.top, this.gridColor, 0, 0);
						}
					}
					...
					{
						context.renderSolidColor(rc.left + spriteX, rc.top + spriteY, rc.right - rc.left, rc.bottom - rc.top, this.cursorColor, 0, 0);
					}
				}
				if (this.bUpdateList)
				{
					for (o = 0; o < this.displayList.size(); o++)
					{
						...
						if (hoPtr != null)
						{
							...
							xObject = Math.floor(this.ho.hoX + (rc.left + rc.right) / 2 - sx + this.rh.rhWindowX - hoPtr.hoImgWidth / 2);
							yObject = Math.floor(this.ho.hoY + (rc.top + rc.bottom) / 2 - sy + this.rh.rhWindowY - hoPtr.hoImgHeight / 2);
...
CRunInventoryScrollBar.SX_SLIDER = 8;
CRunInventoryScrollBar.SY_SLIDER = 8;
...
CRunInventoryScrollBar.prototype =
{
	Initialise:   function (rh, x, y, sx, sy, c, ch, dInventory)
	{
		...
		if (sx > sy)
		{
			...
			this.topArrow.top = y;
			this.topArrow.right = x + CRunInventoryScrollBar.SX_SLIDER;
			this.topArrow.bottom = y + sy;

			this.center.left = x + CRunInventoryScrollBar.SX_SLIDER;
			this.center.top = y;
			this.center.right = x + sx - CRunInventoryScrollBar.SX_SLIDER;
			this.center.bottom = y + sy;

			this.bottomArrow.left = x + sx - CRunInventoryScrollBar.SX_SLIDER;
			this.bottomArrow.top = y;
			this.bottomArrow.right = x + sx;
		...
		else
		{
			...
			this.center.left = x;
			this.center.top = y + CRunInventoryScrollBar.SY_SLIDER;
			this.center.right = x + sx;
			this.center.bottom = y + sy - CRunInventoryScrollBar.SY_SLIDER;

			this.bottomArrow.left = x;
			this.bottomArrow.top = y + sy - CRunInventoryScrollBar.SY_SLIDER;
	...
	SetPosition:  function (p, l, t)
	{
		...
		if (this.total > 0)
		{
			if (this.bHorizontal)
			{
				...
				this.slider.right = Math.floor(Math.min(this.slider.left + (this.length * (this.center.right - this.center.left)) / this.total, this.center.right));
			...
			else
			{
				...
				this.slider.bottom = Math.floor(Math.min(this.slider.top + (this.length * (this.center.bottom - this.center.top)) / this.total, this.center.bottom));
	...
	DrawBar:      function (context)
	{
		...
		{
			context.renderRect(this.center.left, this.center.top, this.center.right - this.center.left, this.center.bottom - this.center.top, this.color, 1, 0, 0);
			var color;
			if (this.zone == CRunInventoryScrollBar.ZONE_SLIDER)
				color = this.colorHilight;
			else
				color = this.color;
			context.renderSolidColor(this.slider.left, this.slider.top, this.slider.right - this.slider.left, this.slider.bottom - this.slider.top, color, 1, 0, 0);
			...
			if (this.bHorizontal)
			{
				if (this.zone == CRunInventoryScrollBar.ZONE_TOPARROW)
					...
					color = this.color;
				for (y = this.topArrow.top; y <= this.topArrow.bottom; y++)
					context.renderLine(this.topArrow.left, Math.floor((this.topArrow.bottom + this.topArrow.top) / 2), this.topArrow.right, y, color, 1, 0, 0);

				if (this.zone == CRunInventoryScrollBar.ZONE_BOTTOMARROW)
					color = this.colorHilight;
				else
					color = this.color;
				for (y = this.bottomArrow.top; y <= this.bottomArrow.bottom; y++)
					context.renderLine(this.bottomArrow.left, y, this.bottomArrow.right, Math.floor((this.bottomArrow.bottom + this.bottomArrow.top) / 2), color, 1, 0, 0);
			}
			else
			{
				var x;
				if (this.zone == CRunInventoryScrollBar.ZONE_TOPARROW)
					...
					color = this.color;
				for (x = this.topArrow.left; x <= this.topArrow.right; x++)
					context.renderLine(x, this.topArrow.bottom, Math.floor((this.topArrow.left + this.topArrow.right) / 2), this.topArrow.top, color, 1, 0, 0);

				if (this.zone == CRunInventoryScrollBar.ZONE_BOTTOMARROW)
					color = this.colorHilight;
				else
					color = this.color;
				for (x = this.bottomArrow.left; x <= this.bottomArrow.right; x++)
					context.renderLine(x, this.bottomArrow.top, Math.floor((this.bottomArrow.left + this.bottomArrow.right) / 2), this.bottomArrow.bottom, color, 1, 0, 0);
			}
		}
	}
}
