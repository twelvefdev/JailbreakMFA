...
CRunKcBoxB.prototype = CServices.extend(new CRunExtension(),
	{
		...
		DisplayObject: function (context, idAppApp, rc, pText, hFnt)
		{
			...
			if ((pText.length != 0) && (this.rData_textColor != CRunKcBoxB.COLOR_NONE))
			{
				...
				if ((this.rData_dwFlags & CRunKcBoxB.FLAG_BUTTON) != 0 &&
					(this.rData_dwFlags & (CRunKcBoxB.FLAG_BUTTON_PRESSED | CRunKcBoxB.FLAG_CHECKED)) != 0 &&
					(this.rData_dwFlags & (CRunKcBoxB.FLAG_BUTTON | CRunKcBoxB.FLAG_CHECKBOX | CRunKcBoxB.FLAG_IMAGECHECKBOX)) != (CRunKcBoxB.FLAG_BUTTON | CRunKcBoxB.FLAG_CHECKBOX | CRunKcBoxB.FLAG_IMAGECHECKBOX))
				{
					textLocation.left += 2;
					textLocation.top += 2;
			...
			if (bDisplayBorder == true)
			{
				if (color1 != CRunKcBoxB.COLOR_NONE)
				{
					...
					context.renderSolidColor(x, y, w - 1, 1, color1);
					context.renderSolidColor(x, y, 1, h - 1, color1);
                }
				if (color2 != CRunKcBoxB.COLOR_NONE)
				{
					...
					context.renderSolidColor(x, y + h - 1, w - 1, 1, color2);
					context.renderSolidColor(x + w - 1, y, 1, h, color2);
                }
			}
		},
